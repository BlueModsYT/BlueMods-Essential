...
export function showTeleportUI(player) {
...
export function showBlockList(player) {
    ...
    form.show(player).then((response) => {
        ...
        if (blockedPlayers.includes(selectedPlayer.id)) {
            tpablocks[player.id] = tpablocks[player.id].filter(id => id !== selectedPlayer.id);
            player.sendMessage(`§7[§b#§7] §aYou have unblocked §e${selectedPlayer.name}§a.`);
            system.run(() => player.runCommand('playsound note.bell @s'));
        } else {
            tpablocks[player.id].push(selectedPlayer.id);
            player.sendMessage(`§7[§b#§7] §cYou have blocked §e${selectedPlayer.name}§c.`);
            system.run(() => player.runCommand('playsound note.bell @s'));
        }
...
export function sendTeleportRequest(sender, target) {
    if (sender.hasTag("isCombat")) {
        return sender.sendMessage("§7[§c!§7] §cYou cannot send teleport requests while in combat!");
    }
    
    if (target.hasTag("isCombat")) {
        return sender.sendMessage(`§7[§c!§7] §c${target.name} is currently in combat and cannot receive requests.`);
    }
    ...
    if (cooldowns[sender.id] && cooldowns[sender.id] > Date.now()) {
        const remainingMs = cooldowns[sender.id] - Date.now();
        const remainingSeconds = Math.ceil(remainingMs / 1000);
        return sender.sendMessage(`§7[§b#§7] §cYou must wait §e${remainingSeconds}s §cbefore sending another request.`);
    }

    sender.sendMessage(`§7[§b#§7] §aRequest sent to §e${target.name}`);
    target.sendMessage(`§7[§b#§7] §e${sender.name} §ahas sent you a teleport request. type §e \"§3!tpa accept §a${main.player}§e\" §ato accept the request.`);
...
export function acceptTeleportRequest(player, sender) {
    const requestKey = Object.keys(playerRequest).find(key => {
        const req = playerRequest[key];
        return req.target === player.id && req.sender === sender.id;
    });

    if (!requestKey) {
        return player.sendMessage(`§7[§b#§7] §cNo pending request from §e${sender.name}§c.`);
    }

    const request = playerRequest[requestKey];
    
    if (!sender) return player.sendMessage("§7[§b#§7] §cThe requester is no longer online.");

    if (player.hasTag("isCombat")) {
        return player.sendMessage("§7[§c!§7] §cYou cannot accept teleport requests while in combat!");
    }
    
    if (sender.hasTag("isCombat")) {
        delete playerRequest[requestKey];
        return player.sendMessage("§7[§c!§7] §cThe requester is now in combat. Request cancelled.");
    }

    player.sendMessage(`§7[§b#§7] §aTeleport request from §e${sender.name} §aaccepted. Teleporting in §e${main.teleportCooldown} §aseconds.`);
    sender.sendMessage(`§7[§b#§7] §e${player.name} §ahas accepted your teleport request.`);

    let countdown = main.teleportCooldown;
    ...
    function tick() {
        ...
        if (player.hasTag("isCombat") || sender.hasTag("isCombat")) {
            sender.sendMessage("§7[§c!§7] §cTeleport cancelled! You or the target entered combat.");
            player.sendMessage("§7[§c!§7] §cTeleport cancelled! You or the requester entered combat.");
        ...
        if (currentPosition.x !== startPosition.x || currentPosition.y !== startPosition.y || currentPosition.z !== startPosition.z) {
            ...
            player.sendMessage("§7[§b#§7] §cTeleport cancelled because the requester moved!");
            system.run(() => sender.runCommand(`playsound random.break @s`));
            teleportCancelled = true;
            delete playerRequest[requestKey];
            return;
...
export function showOutgoingRequests(player) {
    ...
    form.show(player).then((response) => {
        ...
        if (response.selection === outgoing.length) {
            return showTeleportUI(player);
        }
        
        const selectedRequest = outgoing[response.selection];
        if (selectedRequest) {
...
export function showIncomingRequests(player) {
    ...
    form.show(player).then((response) => {
        ...
        if (response.selection === incomingRequests.length) {
            return showTeleportUI(player);
        }

        const selectedRequest = incomingRequests[response.selection];
