...
import { isLored, isDanger, isEnchant, isOperator, isSpawnEgg, isUnknown, isProjectiles, isBundles, isContainers } from "../../items.js";
import { showModerationUI } from "../playerModeration.js";
import { customFormUICodes } from "../../handler/customFormUICodes.js";
import { addLog } from "./playerLogs.js";
...
function saveModules() {
    try { world.setDynamicProperty(MODULE_STATES_KEY, JSON.stringify(main.Modules)); } catch (error) {}
}
...
function checkForNameSpoof(player) {
    if (!isModuleEnabled("nameSpoofCheck")) return false;
    return !validNameRegex.test(player.name.trim());
}
        ...
        for (const player of world.getPlayers()) {
            ...
            for (let i = 0; i < inventory.size; i++) {
                ...
                for (const enchant of enchantments) {
                    if (enchant.level > enchant.type.maxLevel) { modified = true; }
                    else { newEnchantments.push(enchant); }
                }
                if (modified) {
                    addLog(player.name, "Illegal Enchantment", item.typeId.replace("minecraft:", ""));
                    enchantmentComponent.removeAllEnchantments();
                    for (const enchant of newEnchantments) enchantmentComponent.addEnchantment(enchant);
...
function removeBundlesFromInventory(player) {
    if (!isModuleEnabled("removeBundlesInventory")) return;
    ...
    if (removed) {
        addLog(player.name, "Bundle Inventory", "Bundles removed from inventory");
        system.run(() => player.runCommand('playsound random.break @s'));
    }
}
                    ...
                    if (block && block.typeId === "minecraft:crafter") {
                        ...
                        if (itemsDeleted) {
                            ...
                            for (const p of world.getAllPlayers()) {
                                const dist = Math.sqrt(Math.pow(p.location.x - blockX, 2) + Math.pow(p.location.y - checkY, 2) + Math.pow(p.location.z - blockZ, 2));
                                if (dist < nearestDistance) { nearestDistance = dist; nearestPlayer = p.name; }
                            }
                            addLog(nearestPlayer, "Dupe Bundle Crafter", "Tried to put String/Leather in a Crafter");
                        }
                    }
...
function removeFromContainer(container, itemType) {
    ...
    for (let i = 0; i < container.size; i++) {
        const item = container.getItem(i);
        if (item?.typeId === itemType) { container.setItem(i, undefined); removed = true; }
    }
    return removed;
}
                        ...
                        if (container) {
                            for (const itemType of isBundles) {
                                ...
                                if (removed) {
                                    ...
                                    addLog(nearestPlayer, "Dupe Bundle", `${itemType.replace("minecraft:", "")} at ${blockX} ${checkY} ${blockZ}`);
                                }
                            }
                        }
                    }
...
export function hasTimerReachedEnd(targetDate) { return Date.now() >= new Date(targetDate).getTime(); }
...
export function getTime(timerInfo) { return formatTime(new Date(timerInfo.targetDate) - Date.now()); }
...
export function ModulesPanel(player) {
    ...
    form.show(player).then((response) => {
        ...
        } else { showModerationUI(player); return; }
        ModulesPanel(player);
    }).catch((error) => { console.error("Failed to show module states panel:", error); });
}
...
system.runInterval(() => {
    for (const player of world.getAllPlayers()) {
        ...
        if (isModuleEnabled("removeDupeBundles")) checkPlayerAndContainers(player);
        if (isModuleEnabled("removeDupeCrafter")) checkCrafterForBundleRecipe(player);
        if (isModuleEnabled("removeBundlesInventory")) removeBundlesFromInventory(player);
        if (isModuleEnabled("enchantmentCheck")) enchantCheck();
    }
    if (isModuleEnabled("isAgentMob")) {
        try { world.getDimension("overworld").getEntities().forEach(entity => { if (entity.typeId === "minecraft:agent") entity.remove(); }); } catch {}
    }
    if (isModuleEnabled("isCommandBlockMinecart")) {
        try { world.getDimension("overworld").getEntities().forEach(entity => { if (entity.typeId === "minecraft:command_block_minecart") entity.remove(); }); } catch {}
    }
    if (isModuleEnabled("isNPCMob")) {
        try { world.getDimension("overworld").getEntities().forEach(entity => { if (entity.typeId === "minecraft:npc") entity.remove(); }); } catch {}
    }
}, 5);

system.runInterval(() => {
    world.getPlayers({ tag: 'isCombat' }).forEach((player) => {
        if (!CombatDatabase[player.id]) { player.removeTag('isCombat'); return; }
        ...
        CombatDatabase[player.id] = {
            ...
            items: [...Array.from({ length: playerInv.size }, (_, i) => playerInv.getItem(i)).filter(Boolean), ...["Head", "Chest", "Legs", "Feet"].map(slot => player.getComponent("equippable")?.getEquipment(slot)).filter(Boolean)]
        };
    });
}, 20);
...
world.afterEvents.playerSpawn.subscribe((event) => {
    ...
    if (checkForNameSpoof(player)) {
        ...
        system.run(() => player.runCommand('kick @s §cInvalid or spoofed name detected.'));
        world.getPlayers({ tags: ["notify"] }).forEach(admin => {
    ...
    if (isModuleEnabled("receiveCompassOnJoin") && !player.hasTag("new-player")) {
        system.run(() => { player.runCommand("give @s bluemods:itemui"); player.runCommand("tag @s add new-player"); });
        player.sendMessage("§7[§b#§7] §aYou've received a Menu Compass for being a New Player");
    }
    if (isModuleEnabled("removeDupeStackedInventory")) {
        system.runTimeout(() => consolidateInventory(player), 10);
    }
});
...
world.afterEvents.playerLeave.subscribe(({ playerId, playerName }) => {
    ...
    combatData.items?.forEach(item => world.getDimension(combatData.dimension).spawnItem(item, combatData.location));
