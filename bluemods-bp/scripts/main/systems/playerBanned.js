...
import { addLog } from "../moderation/playerModules.js";
...
const BAN_REASON = "§eModerator§7: §bBlueMods §aDataBase\n§eReason§7: §cYou have been banned from this server!";
...
world.afterEvents.playerJoin.subscribe((event) => {
    ...
    system.runTimeout(() => {
        ...
        if (BANNED_PLAYERS.some(name => name.toLowerCase() === playerName.toLowerCase())) {
            system.run(() => {
                player.runCommand(`kick "${playerName}" ${BAN_REASON}`);
            });
            console.warn(`[BanList] Kicked banned player: ${playerName}`);
            addLog(player, "Global Banned", `Permanently Banned from Global Database§r`);
        }
    }, 10);
});
...
system.run(() => {
    for (const player of world.getAllPlayers()) {
        if (BANNED_PLAYERS.some(name => name.toLowerCase() === player.name.toLowerCase())) {
            player.runCommand(`kick "${player.name}" ${BAN_REASON}`);
            console.warn(`[BlueMods-BanList] Kicked banned player: ${player.name}`);
        }
    }
});

console.warn(`[BlueMods-BanList] Loaded ${BANNED_PLAYERS.length} banned players`);